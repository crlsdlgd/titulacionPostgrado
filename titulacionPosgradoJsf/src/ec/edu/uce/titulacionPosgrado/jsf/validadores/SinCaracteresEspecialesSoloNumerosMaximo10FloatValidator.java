...
@FacesValidator("sinCaracteresEspecialesSoloNumerosMaximo10FloatValidator")
public class SinCaracteresEspecialesSoloNumerosMaximo10FloatValidator implements Validator {
	...
	public void validate(FacesContext fc, UIComponent comp, Object valor) throws ValidatorException {
		...
		try {
		     ...
		     if(puntoDecimalUbc==-1){
		    	 if(Float.parseFloat(valorSt)>10){
		    		 FacesUtil.limpiarMensaje();
				   	 FacesMessage msg = new FacesMessage("Error","Calificación máximo sobre 10 puntos");
					 msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					 throw new ValidatorException(msg);
		    	 ...
		    	 if(totalDecimales>2){
					 ...
					 msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					 throw new ValidatorException(msg); 
				 }else if(Float.parseFloat(valorSt)>10){
					 FacesUtil.limpiarMensaje();
					 FacesMessage msg = new FacesMessage("Error","Calificación máximo sobre 10 puntos");
					 msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					 throw new ValidatorException(msg);
