...
public class FechaEstudioValidator implements Validator {
	...
	public void validate(FacesContext fc, UIComponent comp, Object valor) throws ValidatorException {
		...
		EstudianteActaGradoJdbcDto estudiante = (EstudianteActaGradoJdbcDto)comp.getAttributes().get("estudiante");
		...
		if(fechaIng != null && fechaEgr != null && fechaGra != null){
			if(!GeneralesUtilidades.verificarEntreFechas(fechaIng, fechaGra, fechaEgr)){
				FacesMessage msg = new FacesMessage("Error","La fecha de culminación debe estar entre las fechas de ingreso y de posgrado");
			...
			if(fechaIng != null && fechaEgr != null){
				if(GeneralesUtilidades.calcularDiferenciFechas(fechaIng, fechaEgr)<=0){
					FacesMessage msg = new FacesMessage("Error","La fecha de ingreso debe ser menor a la fecha de culminación");
					msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					throw new ValidatorException(msg);
				}
			}
			if(fechaIng != null && fechaGra != null){
				if(GeneralesUtilidades.calcularDiferenciFechas(fechaIng, fechaGra)<=0){
					FacesMessage msg = new FacesMessage("Error","La fecha de ingreso debe ser menor a la fecha de posgrado");
					msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					throw new ValidatorException(msg);
				}
			}
			if(fechaEgr != null && fechaGra != null){
				if(GeneralesUtilidades.calcularDiferenciFechas(fechaEgr, fechaGra)<=0){
					FacesMessage msg = new FacesMessage("Error","La fecha de culminación debe ser menor a la fecha de posgrado");
					msg.setSeverity(FacesMessage.SEVERITY_ERROR);
					throw new ValidatorException(msg);
				}
			}
		...
		if(fechaGra!=null && fechaGra.after(hoy) ){
			FacesUtil.limpiarMensaje();
			FacesMessage msg = new FacesMessage("Error","La fecha de posgrado no puede ser mayor que la fecha actual");
			msg.setSeverity(FacesMessage.SEVERITY_ERROR);
			throw new ValidatorException(msg);
		}
